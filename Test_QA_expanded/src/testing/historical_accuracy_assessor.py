...
from .models import (
    ...
    HISTORICAL_SAMPLE_COLUMNS,
    STATUS_COLUMN,
    HistoricalAccuracyAssessment,
)


class HistoricalAccuracyAssessor:
    def write(self, output_dir: Path) -> None:
        historical_samples = self._load_historical_samples(output_dir)
        assessments = self._analyze_historical_samples(historical_samples)
        analytics_dir = output_dir / "analytics"

        self._write_historical_accuracy_csv(
            analytics_dir / "accuracy_assessment_analytics.csv",
            assessments,
        )
        self._write_historical_accuracy_dashboard(
            analytics_dir / "accuracy_assessment_dashboard.png",
            assessments,
            self._valid_values_by_ammeter(historical_samples),
        )

    def _load_historical_samples(self, output_dir: Path) -> pd.DataFrame:
        ...
        for run_dir in sorted(path for path in samples_dir.iterdir() if path.is_dir()):
            for ammeter_type, sample_path in self._sample_paths_for_run(run_dir).items():
                ...
                if sample_df.empty:
                    continue
                if STATUS_COLUMN not in sample_df.columns:
                    sample_df[STATUS_COLUMN] = ""
                if CURRENT_COLUMN not in sample_df.columns:
                    sample_df[CURRENT_COLUMN] = pd.NA
        ...
        historical_samples[STATUS_COLUMN] = (
            historical_samples[STATUS_COLUMN].fillna("").astype(str).str.strip().str.lower()
        )
        historical_samples[CURRENT_COLUMN] = pd.to_numeric(
            historical_samples[CURRENT_COLUMN], errors="coerce"
        )
        finite_current = historical_samples[CURRENT_COLUMN].map(math.isfinite)
        historical_samples.loc[~finite_current, CURRENT_COLUMN] = pd.NA
        return historical_samples

    def _sample_paths_for_run(self, run_dir: Path) -> Dict[str, Path]:
    ...
    def _analyze_historical_samples(
        ...
        historical_samples["is_valid"] = (
            historical_samples[STATUS_COLUMN].isin(("", "ok"))
            & historical_samples[CURRENT_COLUMN].notna()
        )

        for ammeter_type, sample_df in historical_samples.groupby("ammeter_type", sort=False):
            valid_values = sample_df.loc[sample_df["is_valid"], CURRENT_COLUMN]
    ...
    def _write_historical_accuracy_csv(
        ...
        csv_df = pd.DataFrame(
            [
                asdict(assessment)
                for assessment in sorted(assessments, key=self._assessment_sort_key)
            ],
    ...
    def _valid_values_by_ammeter(self, historical_samples: pd.DataFrame) -> Dict[str, List[float]]:
        if historical_samples.empty:
            return {}
        ...
        return {
            str(ammeter_type): sample_df[CURRENT_COLUMN].tolist()
    ...
    def _write_historical_accuracy_dashboard(
        ...
        output_path.parent.mkdir(parents=True, exist_ok=True)
        sorted_assessments = sorted(assessments, key=self._assessment_sort_key)
        valid_assessments = [
            assessment
        ...
        table_values = [
            [
                self._format_metric(assessment.mean_current_a),
                self._format_metric(assessment.median_current_a),
                self._format_metric(assessment.standard_deviation_a),
                self._format_metric(assessment.minimum_current_a),
                self._format_metric(assessment.maximum_current_a),
